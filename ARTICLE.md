...
原始跳转会在 `libsdmdal.so` 识别到特定内部状态和非零目标亮度时进入拒绝路径。改动这四个字节后，到达该位置的调用会继续执行后面的亮度设置代码，从而解决的是 `libsdmdal.so` 内这一处分支拦截，在 HAL 层面放行 DOZE 状态下的亮度设置。

实机验证表明补丁确实让用户态显示栈从 `Skip` 进入 `Setting display brightness`，亮度请求随后到达 N2 内核驱动。但内核检测到面板处于 LP1 后，仍由 `is_backlight_set_skip()` 拒绝请求，所以屏幕依旧保持黑屏。四字节补丁只打开了第一道门，单独使用不会改善 AOD。若沿底层补丁路线继续处理，必须同时对用户态 HAL 和 N2 内核做放行，再验证面板在 LP1 中动态接收连续 DBV 的安全性。
