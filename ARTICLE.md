...
原始跳转会在 `libsdmdal.so` 识别到特定内部状态和非零目标亮度时进入拒绝路径。改动这四个字节后，到达该位置的调用会继续执行后面的亮度设置代码，因此它解决的是 `libsdmdal.so` 内这一处分支拦截。

实机恢复 AOD 时，亮度命令还要通过 HWC 的 `HWPeripheralDRM::SetPanelBrightness()`。这里会检查面板电源状态和 AOD 图层数量，并继续输出 `Power state ... pending or aod layer 0`，随后直接跳过亮度写入。四字节补丁解决了 `libsdmdal.so` 的局部分支，HWC 的电源状态检查仍然挡住恢复命令，所以屏幕依旧保持黑屏。

沿固件补丁路线继续处理，需要先修改 HWC 对 DOZE 电源状态和 AOD 图层的判断，再确认通过 DRM 送入内核面板驱动的亮度命令能被当前屏幕固件接受。每一层都与具体厂商库、驱动和固件版本绑定，修改范围会从一条指令扩展到整条显示状态链。
